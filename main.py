...
def main():
    ...
    if (int(settings.get('DOUBLE')) == 1):
        pathTrainImage1 = dsConf.get('pathDatasetTrain1')
        pathTestImage1 = dsConf.get('pathDatasetTest1')
        print(pathTrainImage1)
    '''
    ...
    # size_test=int(dsConf.get('sizeTest'))
    ...
    if (int(settings.get('LOAD_NN')) == 0):
        ...
        teacher = HyperoptUnet.trainNN(pathTrainImage, pathTestImage, pathTrainMask, pathTestMask, name_model,
                                       size_test, resize, (shape, shape, ch), attention=attention)
        # teacher = HyperModel.trainNN(param, images)
